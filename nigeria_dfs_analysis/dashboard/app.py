...
if page == "Executive Summary":
    ...
    fig = px.bar(
        ...
        title="Market Sizing Funnel (Assumption)",
    )
    fig.update_layout(showlegend=False, height=320, plot_bgcolor="#F8F9FA")
    ...
    df_e = data["efina"]
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=df_e["year"], y=df_e["banked_pct"], mode="lines+markers", name="Formally Banked (%)"))
    fig.add_trace(go.Scatter(x=df_e["year"], y=df_e["excluded_pct"], mode="lines+markers", name="Financially Excluded (%)"))
    fig.add_trace(go.Scatter(x=df_e["year"], y=df_e["mobile_money_pct"], mode="lines+markers", name="Mobile Money (%)"))
    fig.update_layout(height=400, hovermode="x unified", yaxis_title="% of Adults (15+)", xaxis_title="Year")
    st.plotly_chart(fig, use_container_width=True)
    ...
    fig = px.bar(df_c, x="year", y=col_map[metric], title=f"{metric} - Nigeria")
    fig.update_layout(height=380, plot_bgcolor="#F8F9FA")
    st.plotly_chart(fig, use_container_width=True)
    ...
    fig = go.Figure(go.Bar(
        x=[tam_size, sam_size, som_y1, som_y3],
        y=["TAM", "SAM", "SOM Year 1", "SOM Year 3"],
        orientation="h",
        text=[f"${v:,.0f}M" for v in [tam_size, sam_size, som_y1, som_y3]],
        textposition="outside",
    ))
    fig.update_layout(height=350, xaxis_title="Market Size (USD Millions)", title="Nigeria DFS Market Sizing Funnel")
    st.plotly_chart(fig, use_container_width=True)
    ...
    with col_a:
        fig1 = px.bar(df_r, x=df_r.index.map({0: "Y1", 1: "Y2", 2: "Y3"}), y="total_revenue_usd_m", title="Revenue (USD M)")
        fig1.update_layout(height=280, xaxis_title="Year", plot_bgcolor="#F8F9FA")
        st.plotly_chart(fig1, use_container_width=True)
    with col_b:
        fig2 = px.bar(df_p, x=df_p.index.map({0: "Y1", 1: "Y2", 2: "Y3"}), y="ebitda_usd_m", title="EBITDA (USD M)")
